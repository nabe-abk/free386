...
;/////////////////////////////////////////////////////////////////////////////
%define	int_21h_any	call_V86_int21_iret

	align	4
int21h_table:
	;### function 00h-07h ######
	dd	offset int_21h_00h	;プログラム終了 ret = 0ffh
	dd	offset int_21h_any	;エコー付きキー入力
	dd	offset int_21h_any	;1文字標準出力
	dd	offset int_21h_any	;1文字標準補助入力 (AUX)
	dd	offset int_21h_any	;1文字標準補助出力 (AUX)
	dd	offset int_21h_any	;1文字標準リスト出力 (PRN)
	dd	offset int_21h_any	;直接標準入出力
	dd	offset int_21h_any	;直接(フィルタなし)標準入力

	;### function 08h-0eh ######
	dd	offset int_21h_any	;エコー無しキー入力
	dd	offset int_21h_09h	;文字列の出力
	dd	offset int_21h_0ah	;バッファ付き標準1行入力
	dd	offset int_21h_any	;キーボードステータスチェック
	dd	offset int_21h_any	;バッファクリア & 入力
	dd	offset int_21h_any	;DISK reset (file buffer flash)
	dd	offset int_21h_any	;カレントドライブの変更
...
%endrep
	...
	dd	offset int_21h_any	;カレントドライブ取得
	...
	dd	offset int_21h_notsupp	;FCB / サポートせず
	dd	offset int_21h_notsupp	;FCB
	dd	offset int_21h_any	;日付取得
	dd	offset int_21h_any	;日付設定
	dd	offset int_21h_any	;時刻取得
	dd	offset int_21h_any	;時刻設定
	dd	offset int_21h_any	;ベリファイフラグの セット/リセット
	...
	dd	offset int_21h_any	;CTRL-C 検出状態 設定／取得
	dd	offset int_21h_ret_esbx	;InDOSフラグのアドレス取得
	dd	offset DOS_Extender_fn	;DOS-Extender ファンクション
	dd	offset int_21h_any	;ディスク残り容量取得
	...
	dd	offset int_21h_any	;子プログラムのリターンコード取得
	dd	offset int_21h_ds_edx	;最初に一致するファイルの検索
	dd	offset int_21h_any	;次に一致するファイルの検索
	...
	dd	offset int_21h_unknown	;unknown
	dd	offset int_21h_any	;ベリファイフラグの取得
	dd	offset int_21h_unknown	;unknown
	dd	offset int_21h_56h	;ファイルの移動（リネーム）
	dd	offset int_21h_any	;ファイルの時間情報 取得/設定

	;### function 58h-5fh ######
	dd	offset int_21h_any	;メモリ割り当て方法の変更
	dd	offset int_21h_any	;拡張エラーコードの取得
	dd	offset int_21h_ds_edx	;テンプラリファイルの作成
	dd	offset int_21h_ds_edx	;新規ファイルの作成
	dd	offset int_21h_any	;ファイルアクセスの ロック/アンロック
	dd	offset int_21h_unknown	;unknown
	...
	dd	offset int_21h_unknown	;unknown
	dd	offset int_21h_unknown	;unknown
	dd	offset int_21h_any	;オープン可能な最大ハンドル数の設定
