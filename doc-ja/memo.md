...
また、Open Watcomを使って「Ver2.16なNASM.EXP」を生成（添付）したので、開発が少しだけ楽になりました。0.98と比較すると、short/near jmpの自動切り替え（短縮）が効くとか（Ver0.98は条件分岐がshortで届かないときエラーを吐かれる）、es:[ebx] といった標準的なセレクタ書式が通るとか（Ver0.98は[es:ebx]ではないとダメ）、些細な部分が多いのですがLinux版NASMとの違いを気にする必要がなくなりました。
