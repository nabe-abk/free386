...
FM-TOWNS及びPC-98で動作するパソコン通信ホストプログラム実現のための、
expファイルでネイティブマルチタスク環境（カーネル+シェル）を作成をしようとしていました。
...
そんなわけで、mtask から派生したのが、このFree386プロジェクトです。

幸い mtask を開発する過程で、CPUの知識も、DOS-Extenderの知識も、
アセンブラルーチンも結構ありましたので、それらを活用することで開発を始めました。

そのうちに西暦2000年を過ぎてまして、パソコン通信はすでにオワコン。
そんな中、Free386は FM-TOWNS、PC-9801、PC-AT互換機で
共通のバイナリが動くDOS-Extender環境という目標を掲げます。
